package com.sistema.examenes.iam.interfaces.acl;
...
public class IamContextFacade {
     ...
     */
    public Long createUser(String username, String password, String name, String lastname, String email, String phone) {
        var signUpCommand = new SignUpCommand(username, password, name, lastname, email, phone, List.of(Role.getDefaultRole()));
     ...
     * @return The id of the created user.
     */
    public Long createUser(String username, String password, String name, String lastname, String email, String phone, List<String> roleNames) {
        var roles = roleNames != null ? roleNames.stream().map(Role::toRoleFromName).toList() : new ArrayList<Role>();
        var signUpCommand = new SignUpCommand(username, password, name, lastname, email, phone, roles);
        var result = userCommandService.handle(signUpCommand);
        if (result.isEmpty()) return 0L;
        return result.get().getId();
    }
